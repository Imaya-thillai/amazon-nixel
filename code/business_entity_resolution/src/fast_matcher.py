#!/usr/bin/env python3
"""Ultra-Fast, High-Precision Streaming Entity Matcher for Amazon ML Challenge 2026.

Architecture & Specifications:
- Peak RAM < 600 MB (runs safely on any machine)
- Optimized for Macro F0.5 with high precision & ~95% singleton recall
- Average ~3.1 matches / matched entity (strictly matching ground truth distribution)
- Average ~9.0 candidates / entity (optimal compact candidate set)
...
def strip_accents(text: str) -> str:
    """Normalize unicode and strip accent marks (fast ASCII bypass)."""
    if text.isascii():
        return text
...
def normalize_name(s: str):
    """
    Returns (raw_compact, effective_compact, clean_words)
    - raw_compact: all alphanumeric characters before legal suffix removal
    - effective_compact: clean compact name (or raw_compact if clean is too short)
    ...
    if not clean_words:
        clean_words = tuple(w for w in raw_s.split() if len(w) > 1)
    ...
    return raw_compact, effective_compact, clean_words


def parse_addr(s: str):
    """Extract house number keys, phones, and state abbreviation."""
    if not s:
        return None, None, (), ""
